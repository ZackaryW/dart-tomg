...
The `tomgen` executable SHALL expose `generate`, `build`, and `clean`. `generate`
SHALL perform phase-one model generation only. `build` SHALL complete phase one
and then invoke `dart run build_runner build` from the package root, forwarding
arguments after `--`, standard input/output, and the child exit status. It SHALL
not start phase two after a phase-one failure. `clean` SHALL remove only
unchanged phase-one files proven by the ownership manifest and SHALL NOT remove
build_runner-owned `.g.dart` files.
